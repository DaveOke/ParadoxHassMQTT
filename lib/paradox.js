...
module.exports = class ParadoxConnector {
	...
	TranslateCommand(command) {
		...
		if (command.indexOf("G") === 0) {
			...
			this._mqttClient.publish("paradox_evo/alarm/area1", state);
			...
			if (command.split("&")[1] === "ok")
				this._mqttClient.publish("paradox_evo/alarm/area1", this._msg);
		}
                else if(command.indexOf("AD") === 0) {
			if (command.split("&")[1] === "ok")
                        	this._mqttClient.publish("paradox_evo/alarm/area1", "disarmed");
                }
